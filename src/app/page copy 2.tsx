...
type StartType = 'Automático' | 'Manual' | 'Deshabilitado'
...
export default function Component() {
  ...
  const fetchStartType = useCallback(async () => {
    ...
    try {
      ...
      if (!response.error) {
        ...
        setError(response.mensaje || 'Error al obtener el tipo de inicio')
      }
    } catch {
      setError('Error en la comunicación con Tauri')
    }
  ...
  const handleChangeStartType = async () => {
    ...
    try {
      ...
      const response = JSON.parse(result)
      if (response.error) {
        setError(response.mensaje || 'Error al cambiar el tipo de inicio')
      } else {
          ...
          <div className="flex flex-col space-y-2">
                ...
                <PopoverContent className="w-[200px] p-0">
                  <Command>
                    ...
                    <CommandList>
                      ...
                      <CommandGroup>
                        {(
                          ['Automático', 'Manual', 'Deshabilitado'] as const
                        ).map((type) => (
                          <CommandItem
                            key={type}
                            value={type}
            ...
            {currentStartType && (
              <p className="text-sm text-green-600">
                Tipo de inicio actual:{' '}
                <span className="font-semibold">{currentStartType}</span>
              </p>
            )}
          </div>
        )}
